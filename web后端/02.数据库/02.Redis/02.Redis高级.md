...
**一条正确指令，两条语法错误指令**， 当执行exec后，会直接返回错误，正确的命令也不会执行。
...
```
RDB方式会出现数据丢失的问题，对于这个问题，可以通过Redis中另外一种持久化方式解决：
```
...
1. **自动触发**：当AOF文件大小超过配置的阈值时自动触发重写
2. **手动触发**：通过`bgrewriteaof`命令手动触发
...
CLUSTER SETSLOT  STABLE // 取消对槽 slot 的导入（```
import```null
）或者迁移（migrate）。
//键
...
```
对于这些策略各自的含义，我们还需要一点前置知识的铺垫，这里我们可以看到两个名称：
```
...
**LRU**和**TTL**相关策略，每次触发时，redis会默认从5个key中一个key符合条件的key进行删除。如果要修改的话，可以修改**redis.conf**中**maxmemory-samples**属性值
