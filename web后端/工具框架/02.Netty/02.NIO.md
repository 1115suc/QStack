...
> 🖥️ **获取 CPU 核心数**：`Runtime.getRuntime().availableProcessors()`

**注意事项：**

- ⚠️ **Docker 容器问题**：如果工作在 Docker 容器下，因为容器不是物理隔离的，会拿到物理 CPU 个数，而不是容器申请时的个数
...
- ⚠️ **数据可能丢失**：server 这边的 receive 方法会将接收到的数据存入 byte buffer，但如果数据报文超过 buffer 大小，多出来的数据会被默默抛弃
- ⚡ **速度快**：没有连接建立和维护的开销
- 📦 **面向报文**：保留消息边界
...
| 🐧 **Linux**   | 2.6 版本引入 AIO              | ⚠️ 底层用多路复用模拟，性能没有优势 |
...
> ⚠️ **重要**：默认文件 AIO 使用的线程都是守护线程，所以最后要执行 `System.in.read()` 以避免守护线程意外结束。
